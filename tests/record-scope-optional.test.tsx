...
type RecordScopeData = number[];

function RecordScopeButton(props: { scope: IRecordScope<RecordScopeData> }) {
  const handleClick = useCallback(() => {
    const value = props.scope.get('one')?.[0] ?? 1;
    props.scope.set('one', [value * 10]);
...
test('useRecordScope', () => {
  ...
  expect(one[0]).toEqual(1);
  two = getLabelTwoState(tree);
  expect(two[0]).toEqual(2);
...
test('RecordScope', () => {
  ...
  renderer.act(() => {
    scope.set('one', [scope.get('one')![0] * 10]);
    scope.delete('two');
    component.update(
      <>
        <RecordScopeLabel id="one" scope={scope} />
